...
const CarouselSection = () => {
  ...
  return (
    <div className="my-20 bg-primary h-auto md:h-[500px]">
      <div className="bg-accent/20 grid lg:grid-cols-1 grid-cols-1">
        ...
        <Slider {...settings}>
          {testimonials.map((testimonial, index) => (
            <div key={index} className="px-2 sm:px-4">
              <div className="p-4 sm:p-6 bg-white border border-gray-500 text-center rounded-lg">
                <div className="flex text-left mb-2 sm:mb-4">
                  {[...Array(testimonial.rating)].map((_, i) => (
                    <svg
                      key={i}
                      className="text-2xl sm:w-5 sm:h-5 text-yellow-500"
                      fill="currentColor"
                      viewBox="0 0 20 20"
                    >
                ...
                <p className="text-base sm:text-lg text-left mb-2 sm:mb-4">
                  "{testimonial.text}"
                </p>
                <h3 className="text-lg sm:text-[15px] text-left font-bold">
                  {testimonial.author}
                </h3>
                <p className="text-gray-600 text-[13px] text-left">
